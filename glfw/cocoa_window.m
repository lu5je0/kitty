...
polymorphic_string_as_utf8(id string) {
    ...
    NSString *characters;
    if ([string isKindOfClass:[NSAttributedString class]]) characters = [string string];
    else characters = (NSString *)string;
...
get_first_codepoint(UniChar *utf16, UniCharCount num) {
    ...
    if (CFStringIsSurrogateHighCharacter(*utf16) && num > 1 && CFStringIsSurrogateLowCharacter(utf16[1]))
        return CFStringGetLongCharacterForSurrogatePair(utf16[0], utf16[1]);
    return 0;
}
...
//
static NSUInteger
getStyleMask(_GLFWwindow *window) {
...
_glfwRestartDisplayLinks(void) {
    ...
    for (window = _glfw.windowListHead; window; window = window->next) {
        if (window->ns.renderFrameRequested && window->ns.renderFrameCallback) { requestRenderFrame(window, window->ns.renderFrameCallback); }
    }
}
...
//
static void
hideCursor(_GLFWwindow *window UNUSED) {
...
//
static void
showCursor(_GLFWwindow *window UNUSED) {
    if (_glfw.ns.cursorHidden) {
        [NSCursor unhide];
...
updateCursorImage(_GLFWwindow *window) {
    if (window->cursorMode == GLFW_CURSOR_NORMAL) {
        ...
        if (window->cursor) [(NSCursor *)window->cursor->ns.object set];
        else [[NSCursor arrowCursor] set];
    } else hideCursor(window);
}

// Apply chosen cursor mode to a focused window
//
static void
updateCursorMode(_GLFWwindow *window) {
...
acquireMonitor(_GLFWwindow *window) {
    ...
    const NSRect frame = NSMakeRect(bounds.origin.x, _glfwTransformYNS(bounds.origin.y + bounds.size.height - 1), bounds.size.width, bounds.size.height);
...
- (void)windowDidResize:(NSNotification *)notification {
    ...
    if (window->ns.resizeCallback && !is_screen_change && !is_fullscreen && window->ns.live_resize_in_progress) window->ns.resizeCallback((GLFWwindow *)window);
}
...
- (instancetype)initWithGlfwWindow:(_GLFWwindow *)initWindow {
    ...
    if (self != nil) {
        ...
        NSMutableArray *types = [NSMutableArray arrayWithObjects:NSPasteboardTypeFileURL,
                                                                 ...
                                                                 UTTypeContent.identifier,
                                                                 mime_to_uti(tab_mime),
                                                                 mime_to_uti(window_mime),
                                                                 nil];
...
- (void)updateLayer {
    ...
    if (window->context.client != GLFW_NO_API) {
        @try {
            ...
            _glfwInputError(GLFW_PLATFORM_ERROR, "Failed to update NSGL Context object with error: %s (%s)", [[e name] UTF8String], [[e reason] UTF8String]);
        }
    }
...
- (void)mouseDown:(NSEvent *)event {
    if (!window) return;
    _glfwInputMouseClick(window, GLFW_MOUSE_BUTTON_LEFT, GLFW_PRESS, translateFlags([event modifierFlags]));
}
...
- (void)mouseUp:(NSEvent *)event {
    if (!window) return;
    _glfwInputMouseClick(window, GLFW_MOUSE_BUTTON_LEFT, GLFW_RELEASE, translateFlags([event modifierFlags]));
}
...
- (void)rightMouseDown:(NSEvent *)event {
    if (!window) return;
    _glfwInputMouseClick(window, GLFW_MOUSE_BUTTON_RIGHT, GLFW_PRESS, translateFlags([event modifierFlags]));
}
...
- (void)rightMouseUp:(NSEvent *)event {
    if (!window) return;
    _glfwInputMouseClick(window, GLFW_MOUSE_BUTTON_RIGHT, GLFW_RELEASE, translateFlags([event modifierFlags]));
}

- (void)otherMouseDown:(NSEvent *)event {
    if (!window) return;
    _glfwInputMouseClick(window, (int)[event buttonNumber], GLFW_PRESS, translateFlags([event modifierFlags]));
}
...
- (void)otherMouseUp:(NSEvent *)event {
    if (!window) return;
    _glfwInputMouseClick(window, (int)[event buttonNumber], GLFW_RELEASE, translateFlags([event modifierFlags]));
}
...
- (void)viewDidChangeBackingProperties {
    ...
    if (xscale != window->ns.xscale || yscale != window->ns.yscale) {
        ...
        if (window->ns.retina && window->ns.layer) [window->ns.layer setContentsScale:[window->ns.object backingScaleFactor]];
    }
}
...
- (void)updateTrackingAreas {
    if (window && [window->ns.object areCursorRectsEnabled]) [window->ns.object disableCursorRects];
    if (trackingArea != nil) {
        [self removeTrackingArea:trackingArea];
        [trackingArea release];
    }

    const NSTrackingAreaOptions options = NSTrackingMouseEnteredAndExited | NSTrackingActiveAlways | NSTrackingEnabledDuringMouseDrag | NSTrackingCursorUpdate |
...
convert_cocoa_to_carbon_modifiers(NSUInteger flags) {
    ...
    if (flags & NSEventModifierFlagCapsLock) mods |= alphaLock;
...
convert_utf16_to_utf8(UniChar *src, UniCharCount src_length, char *dest, size_t dest_sz) {
    CFStringRef string = CFStringCreateWithCharactersNoCopy(kCFAllocatorDefault, src, src_length, kCFAllocatorNull);
    CFStringGetCString(string, dest, dest_sz, kCFStringEncodingUTF8);
    CFRelease(string);
}
...
#define UPDATE_PRE_EDIT_TEXT                               \
    ...
    const bool process_text =
        !_glfw.ignoreOSKeyboardProcessing && (!window->ns.textInputFilterCallback || window->ns.textInputFilterCallback(key, mods, keycode, flags) != 1) && !ime_disabled;
    _glfw.ns.text[0] = 0;
    ...
    if (!_glfw.ns.unicodeData) {
        ...
        debug_input(
            "\x1b[31mPress:\x1b[m native_key: 0x%x (%s) glfw_key: 0x%x %schar_count: %lu deadKeyState: %u repeat: %d ",
            keycode,
            safe_name_for_keycode(keycode),
            key,
            format_mods(mods),
            char_count,
            window->ns.deadKeyState,
...
- (void)flagsChanged:(NSEvent *)event {
    ...
    const bool process_text = !_glfw.ignoreOSKeyboardProcessing &&
                              (!window->ns.textInputFilterCallback || window->ns.textInputFilterCallback(key, mods, keycode, modifierFlags) != 1) && filter_result != 2;
...
#define action_for(modname, target_mask, other_mask, either_mask)                                                          \
    ...
    switch (key) {
        case GLFW_FKEY_CAPS_LOCK:
            mod_name = "capslock";
            action = modifierFlags & NSEventModifierFlagCapsLock ? GLFW_PRESS : GLFW_RELEASE;
            break;
...
#undef action_for
    ...
    debug_input(
        "\x1b[33mflagsChanged:\x1b[m modifier: %s native_key: 0x%x (%s) glfw_key: 0x%x %s\n",
        mod_name,
        keycode,
        safe_name_for_keycode(keycode),
        key,
...
#define add_mime(uti)                                                      \
    ...
    for (NSPasteboardItem *item in pasteboard.pasteboardItems) {
        for (NSPasteboardType uti in item.types) { add_mime(uti); }
    }
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
    ...
    if (d->copy_mimes) {
        ...
        for (size_t i = 0; i < num_accepted; i++) _glfwPlatformRequestDropData(window, d->copy_mimes[i]);
    }
...
_glfwPlatformReadAvailableDropData(GLFWwindow *w, GLFWDropEvent *ev, char *buffer, size_t capacity) {
    ...
    if (x == nil && d->in_progress_drop.promises_loaded && d->in_progress_drop.path_map != nil) x = d->in_progress_drop.path_map[@(mime)];
...
- (void)setMarkedText:(id)string selectedRange:(NSRange)selectedRange replacementRange:(NSRange)replacementRange {
    ...
    if ([string isKindOfClass:[NSAttributedString class]]) {
        if (((NSMutableAttributedString *)string).length == 0) {
            [self unmarkText];
            return;
        }
        [markedText release];
...
- (void)updateIMEStateFor:(GLFWIMEUpdateType)which focused:(bool)focused {
...
- (NSRect)firstRectForCharacterRange:(NSRange)range actualRange:(NSRangePointer)actualRange {
    ...
    if (_glfw.callbacks.get_ime_cursor_position) {
        ...
        if (window && _glfw.callbacks.get_ime_cursor_position((GLFWwindow *)window, &ev)) {
            ...
            const NSRect rectInView = NSMakeRect(left, frame.size.height - top - cellHeight, cellWidth, cellHeight);
...
- (BOOL)isAccessibilitySelectorAllowed:(SEL)selector {
    ...
    if (selector == @selector(accessibilityWindow) || selector == @selector(accessibilityParent) || selector == @selector(accessibilityPosition) ||
        selector == @selector(setAccessibilityPosition:) || selector == @selector(accessibilitySize) || selector == @selector(setAccessibilitySize:))
        return YES;
...
- (id)validRequestorForSendType:(NSString *)sendType returnType:(NSString *)returnType {
    if ((!sendType || [sendType isEqual:NSPasteboardTypeString] || [sendType isEqual:@"NSStringPboardType"]) &&
...
// For example, open System Settings->Keyboard->Keyboard Shortcuts->Services->Text, enable `Convert Text to Full Width`, select some text and execute the
// service.
...
update_titlebar_button_visibility_after_fullscreen_transition(_GLFWwindow *w, bool traditional, bool made_fullscreen) {
    ...
    if (w->ns.titlebar_hidden) {
        ...
        if (!traditional) { button_hidden = (BOOL)!made_fullscreen; }
...
createNativeWindow(_GLFWwindow *window, const _GLFWwndconfig *wndconfig, const _GLFWfbconfig *fbconfig) {
    ...
    } else contentRect = NSMakeRect(0, 0, wndconfig->width, wndconfig->height);

    window->ns.object = [[GLFWWindow alloc] initWithGlfwWindow:contentRect styleMask:getStyleMask(window) backing:NSBackingStoreBuffered initWindow:window];
    ...
    if (window->monitor) [window->ns.object setLevel:NSMainMenuWindowLevel + 1];
    else {
        [(NSWindow *)window->ns.object center];
        CGRect screen_frame = [[(NSWindow *)window->ns.object screen] frame];
        if (CGRectContainsPoint(screen_frame, _glfw.ns.cascadePoint) || CGPointEqualToPoint(CGPointZero, _glfw.ns.cascadePoint)) {
            _glfw.ns.cascadePoint = NSPointToCGPoint([window->ns.object cascadeTopLeftFromPoint:NSPointFromCGPoint(_glfw.ns.cascadePoint)]);
        } else {
    ...
    if (strlen(wndconfig->ns.frameName)) [window->ns.object setFrameAutosaveName:@(wndconfig->ns.frameName)];
...
_glfwPlatformCreateWindow(
    ...
    if (ctxconfig->client != GLFW_NO_API) {
        if (ctxconfig->source == GLFW_NATIVE_CONTEXT_API) {
            if (!_glfwInitNSGL()) return false;
            if (!_glfwCreateContextNSGL(window, ctxconfig, fbconfig)) return false;
...
#define C(x)                     \
    ...
    C(NSTornOffMenuWindowLevel);
    C(NSMainMenuWindowLevel);
    C(NSStatusWindowLevel);
    C(NSModalPanelWindowLevel);
    C(NSPopUpMenuWindowLevel);
    C(NSScreenSaverWindowLevel);
    C(kCGBaseWindowLevel);
    C(kCGMinimumWindowLevel);
    C(kCGDesktopWindowLevel);
    C(kCGBackstopMenuLevel);
    C(kCGNormalWindowLevel);
    C(kCGFloatingWindowLevel);
    C(kCGTornOffMenuWindowLevel);
    C(kCGDockWindowLevel);
    C(kCGMainMenuWindowLevel);
    C(kCGStatusWindowLevel);
    C(kCGModalPanelWindowLevel);
    C(kCGPopUpMenuWindowLevel);
    C(kCGDraggingWindowLevel);
    C(kCGScreenSaverWindowLevel);
    C(kCGMaximumWindowLevel);
    C(kCGOverlayWindowLevel);
    C(kCGHelpWindowLevel);
    C(kCGUtilityWindowLevel);
    C(kCGDesktopIconWindowLevel);
    C(kCGAssistiveTechHighWindowLevel);
    C(kCGCursorWindowLevel);
    C(kCGNumberOfWindowLevelKeys);
#undef C
    return false;
}
...
screen_for_window_center(_GLFWwindow *window) {
    ...
    for (NSScreen *screen in [NSScreen screens]) {
        if (NSPointInRect(windowCenter, [screen frame])) { return screen; }
    }
...
#define nswindow window->ns.object
    ...
    if (config.type == GLFW_LAYER_SHELL_BACKGROUND || config.edge == GLFW_EDGE_CENTER) {
        ...
        height = NSHeight(screen.frame) - menubar_height + 1;
    ...
    [nswindow
        setCollectionBehavior:(NSWindowCollectionBehaviorCanJoinAllSpaces | NSWindowCollectionBehaviorStationary | NSWindowCollectionBehaviorIgnoresCycle)];
...
_glfwPlatformSetWindowIcon(_GLFWwindow *window UNUSED, int count UNUSED, const GLFWimage *images UNUSED) {
    _glfwInputError(GLFW_FEATURE_UNAVAILABLE, "Cocoa: Regular windows do not have icons on macOS");
}
...
_glfwPlatformGetWindowPos(_GLFWwindow *window, int *xpos, int *ypos) {
    ...
    if (xpos) *xpos = (int)contentRect.origin.x;
    if (ypos) *ypos = (int)_glfwTransformYNS(contentRect.origin.y + contentRect.size.height - 1);
}

void
_glfwPlatformSetWindowPos(_GLFWwindow *window, int x, int y) {
    const NSRect contentRect = get_window_size_without_border_in_logical_pixels(window);
...
_glfwPlatformSetWindowSizeLimits(_GLFWwindow *window, int minwidth, int minheight, int maxwidth, int maxheight) {
    if (minwidth == GLFW_DONT_CARE || minheight == GLFW_DONT_CARE) [window->ns.object setContentMinSize:NSMakeSize(0, 0)];
    else [window->ns.object setContentMinSize:NSMakeSize(minwidth, minheight)];

    if (maxwidth == GLFW_DONT_CARE || maxheight == GLFW_DONT_CARE) [window->ns.object setContentMaxSize:NSMakeSize(DBL_MAX, DBL_MAX)];
    else [window->ns.object setContentMaxSize:NSMakeSize(maxwidth, maxheight)];
}

void
_glfwPlatformSetWindowAspectRatio(_GLFWwindow *window, int numer, int denom) {
    if (numer == GLFW_DONT_CARE || denom == GLFW_DONT_CARE) [window->ns.object setResizeIncrements:NSMakeSize(1.0, 1.0)];
    else [window->ns.object setContentAspectRatio:NSMakeSize(numer, denom)];
}

void
_glfwPlatformSetWindowSizeIncrements(_GLFWwindow *window, int widthincr, int heightincr) {
...
_glfwPlatformGetWindowFrameSize(_GLFWwindow *window, int *left, int *top, int *right, int *bottom) {
    ...
    if (left) *left = (int)(contentRect.origin.x - frameRect.origin.x);
    if (top) *top = (int)(frameRect.origin.y + frameRect.size.height - contentRect.origin.y - contentRect.size.height);
    if (right) *right = (int)(frameRect.origin.x + frameRect.size.width - contentRect.origin.x - contentRect.size.width);
    if (bottom) *bottom = (int)(contentRect.origin.y - frameRect.origin.y);
}
...
_glfwPlatformRestoreWindow(_GLFWwindow *window) {
    if ([window->ns.object isMiniaturized]) [window->ns.object deminiaturize:nil];
    else if ([window->ns.object isZoomed]) [window->ns.object zoom:nil];
}
...
_glfwPlatformShowWindow(_GLFWwindow *window, bool move_to_active_screen) {
    ...
    if (is_background) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{ weakSelf.collectionBehavior = old; });
    }
}
...
_glfwPlatformSetWindowMonitor(_GLFWwindow *window, _GLFWmonitor *monitor, int xpos, int ypos, int width, int height, int refreshRate UNUSED) {
    if (window->monitor == monitor) {
        if (monitor) {
            ...
            const NSRect contentRect = NSMakeRect(xpos, _glfwTransformYNS(ypos + height - 1), width, height);
            const NSRect frameRect = [window->ns.object frameRectForContentRect:contentRect styleMask:getStyleMask(window)];
    ...
    if (window->monitor) {
        ...
        if (window->floating) [window->ns.object setLevel:NSFloatingWindowLevel];
        else [window->ns.object setLevel:NSNormalWindowLevel];
...
int
_glfwPlatformWindowOccluded(_GLFWwindow *window) {
...
_glfwPlatformWindowHovered(_GLFWwindow *window) {
    ...
    if ([NSWindow windowNumberAtPoint:point belowWindowWithWindowNumber:0] != [window->ns.object windowNumber]) { return false; }
...
_glfwPlatformSetWindowFloating(_GLFWwindow *window, bool enabled) {
    if (enabled) [window->ns.object setLevel:NSFloatingWindowLevel];
    else [window->ns.object setLevel:NSNormalWindowLevel];
}
...
_glfwPlatformSetRawMouseMotion(_GLFWwindow *window UNUSED, bool enabled UNUSED) {
    _glfwInputError(GLFW_FEATURE_UNIMPLEMENTED, "Cocoa: Raw mouse motion not yet implemented");
}
...
const char *
_glfwPlatformGetNativeKeyName(int keycode) {
...
int
_glfwPlatformGetNativeKeyForKey(uint32_t glfw_key) {
...
int
_glfwPlatformCreateCursor(_GLFWcursor *cursor, const GLFWimage *image, int xhot, int yhot, int count) {
    NSImage *native;
    ...
    [native release];
    if (cursor->ns.object == nil) return false;
    return true;
}
...
load_hidden_system_cursor(NSString *name, SEL fallback) {
    ...
    NSString *cursorPath = [@"/System/Library/Frameworks/ApplicationServices.framework/Versions/A/Frameworks/HIServices.framework/Versions/A/Resources/cursors"
    ...
    if ((image == nil) || (image.isValid == NO)) { return [NSCursor performSelector:fallback]; }
    ...
    if (frames > 1) {
        ...
        if (cropped == nil) { return [NSCursor performSelector:fallback]; }
...
bool
_glfwPlatformIsFullscreen(_GLFWwindow *w, unsigned int flags) {
    NSWindow *window = w->ns.object;
    bool traditional = !(flags & 1);
    if (traditional) {
        if (@available(macOS 10.15.7, *)) return w->ns.in_traditional_fullscreen;
    }
...
_glfwUpdateNotchCover(_GLFWwindow *w) {
    ...
    if (@available(macOS 12.0, *)) {
        ...
        notchView.layer.backgroundColor =
            [NSColor colorWithSRGBRed:((c >> 16) & 0xFF) / 255.0 green:((c >> 8) & 0xFF) / 255.0 blue:(c & 0xFF) / 255.0 alpha:a].CGColor;
...
bool
_glfwPlatformToggleFullscreen(_GLFWwindow *w, unsigned int flags) {
    NSWindow *window = w->ns.object;
        ...
        if (@available(macOS 10.15.7, *)) {
            ...
            if (!w->ns.in_traditional_fullscreen) {
                ...
                if (!ignore_safe_area_insets) {
                    if (@available(macOS 12.0, *)) { screenFrame.size.height -= window.screen.safeAreaInsets.top; }
                }
...
_glfwPlatformGetClipboard(GLFWClipboardType clipboard_type, const char *mime_type, GLFWclipboardwritedatafun write_data, void *object) {
    ...
    if (t != nil) {
        ...
        /* NSLog(@"data: %@", data); */
        if (data != nil && data.length > 0) { write_data(object, data.bytes, data.length); }
    }
}
...
_glfwPlatformSetClipboard(GLFWClipboardType t) {
    ...
    for (size_t i = 0; i < _glfw.clipboard.num_mime_types; i++) { [ptypes addObject:mime_to_uti(_glfw.clipboard.mime_types[i])]; }
...
_glfwPlatformGetEGLPlatform(EGLint **attribs) {
    if (_glfw.egl.ANGLE_platform_angle) {
        ...
        if (_glfw.egl.ANGLE_platform_angle_metal) {
            if (_glfw.hints.init.angleType == GLFW_ANGLE_PLATFORM_TYPE_METAL) type = EGL_PLATFORM_ANGLE_TYPE_METAL_ANGLE;
        }
...
_glfwPlatformCreateWindowSurface(VkInstance instance, _GLFWwindow *window, const VkAllocationCallbacks *allocator, VkSurfaceKHR *surface) {
    ...
    if (window->ns.retina) [window->ns.layer setContentsScale:[window->ns.object backingScaleFactor]];
    ...
    if (_glfw.vk.EXT_metal_surface) {
        ...
        if (!vkCreateMetalSurfaceEXT) {
            _glfwInputError(GLFW_API_UNAVAILABLE, "Cocoa: Vulkan instance missing VK_EXT_metal_surface extension");
            return VK_ERROR_EXTENSION_NOT_PRESENT;
        }
        ...
        if (!vkCreateMacOSSurfaceMVK) {
            _glfwInputError(GLFW_API_UNAVAILABLE, "Cocoa: Vulkan instance missing VK_MVK_macos_surface extension");
            return VK_ERROR_EXTENSION_NOT_PRESENT;
        }
    ...
    if (err) { _glfwInputError(GLFW_PLATFORM_ERROR, "Cocoa: Failed to create Vulkan surface: %s", _glfwGetVulkanResultString(err)); }
...
GLFWAPI id
glfwGetCocoaWindow(GLFWwindow *handle) {
...
GLFWAPI GLFWcocoatextinputfilterfun
glfwSetCocoaTextInputFilter(GLFWwindow *handle, GLFWcocoatextinputfilterfun callback) {
    _GLFWwindow *window = (_GLFWwindow *)handle;
    _GLFW_REQUIRE_INIT_OR_RETURN(nil);
...
GLFWAPI GLFWhandleurlopen
glfwSetCocoaURLOpenCallback(GLFWhandleurlopen callback) {
    _GLFW_REQUIRE_INIT_OR_RETURN(nil);
...
GLFWAPI GLFWcocoatogglefullscreenfun
glfwSetCocoaToggleFullscreenIntercept(GLFWwindow *handle, GLFWcocoatogglefullscreenfun callback) {
    _GLFWwindow *window = (_GLFWwindow *)handle;
    _GLFW_REQUIRE_INIT_OR_RETURN(nil);
...
GLFWAPI void
glfwCocoaRequestRenderFrame(GLFWwindow *w, GLFWcocoarenderframefun callback) {
    requestRenderFrame((_GLFWwindow *)w, callback);
}

GLFWAPI GLFWcocoarenderframefun
glfwCocoaSetWindowResizeCallback(GLFWwindow *w, GLFWcocoarenderframefun cb) {
    _GLFWwindow *window = (_GLFWwindow *)w;
...
- (NSArray<NSView *> *)viewsWithIdentifier:(NSUserInterfaceItemIdentifier)identifier {
    ...
    if ([self.identifier isEqual:identifier]) { [result addObject:self]; }
    for (NSView *sub in self.subviews) { [result addObjectsFromArray:[sub viewsWithIdentifier:identifier]]; }
    return result;
}
...
static void
clear_title_bar_background_views(NSWindow *window) {
...
#define tc window->ns.last_applied_titlebar_settings.color
        ...
        NSAppearance *light_appearance =
            is_transparent ? [NSAppearance appearanceNamed:NSAppearanceNameVibrantLight] : [NSAppearance appearanceNamed:NSAppearanceNameAqua];
        NSAppearance *dark_appearance =
            is_transparent ? [NSAppearance appearanceNamed:NSAppearanceNameVibrantDark] : [NSAppearance appearanceNamed:NSAppearanceNameDarkAqua];
        if (use_system_color) {
            switch (system_color) {
                case 1: appearance = light_appearance; break;
                case 2: appearance = dark_appearance; break;
            }
        } else {
        ...
        debug(
            ...
            decorations_desc,
            window->decorated,
            window->ns.last_applied_titlebar_settings.transparent,
            tc.was_set,
            show_text_in_titlebar,
            window->ns.titlebar_hidden,
...
glfwGetCocoaKeyEquivalent(uint32_t glfw_key, int glfw_mods, int *cocoa_mods) {
    *cocoa_mods = 0;
    if (glfw_mods & GLFW_MOD_SHIFT) *cocoa_mods |= NSEventModifierFlagShift;
    if (glfw_mods & GLFW_MOD_CONTROL) *cocoa_mods |= NSEventModifierFlagControl;
    if (glfw_mods & GLFW_MOD_ALT) *cocoa_mods |= NSEventModifierFlagOption;
    if (glfw_mods & GLFW_MOD_SUPER) *cocoa_mods |= NSEventModifierFlagCommand;
...
glfwCocoaCycleThroughOSWindows(bool backwards) {
    ...
    for (NSWindow *window in allWindows) {
        ...
        if (windowFrame.size.width > 0 && windowFrame.size.height > 0 && !window.isMiniaturized && window.isVisible &&
            [window isMemberOfClass:[GLFWWindow class]])
            [filteredWindows addObject:window];
    }
    if (filteredWindows.count < 2) return;
    [filteredWindows sortUsingComparator:^NSComparisonResult(NSWindow *a, NSWindow *b) { return [@(a.windowNumber) compare:@(b.windowNumber)]; }];
...
//
float
_glfwTransformYNS(float y) {
    return CGDisplayBounds(CGMainDisplayID()).size.height - y - 1;
}

void
_glfwCocoaPostEmptyEvent(void) {
...
NSStringFromDragOperation(NSDragOperation operation) {
    ...
    if (descriptions.count == 0) { return [NSString stringWithFormat:@"Unknown (%lu)", (unsigned long)operation]; }
...
- (void)draggingSession:(NSDraggingSession *)session endedAtPoint:(NSPoint)screenPoint operation:(NSDragOperation)operation {
    ...
    if (window) {
        ...
        switch (operation) {
            ...
            case NSDragOperationLink: ev.action = GLFW_DRAG_OPERATION_COPY; break;
            case NSDragOperationMove:
            case NSDragOperationDelete: ev.action = GLFW_DRAG_OPERATION_MOVE; break;
        ...
        switch (operation) {
            ...
            default: ev.type = GLFW_DRAG_DROPPED; break;
        }
...
schedule_drag_finish_timer(void) {
    ...
    drag_finish_timer = [NSTimer scheduledTimerWithTimeInterval:DRAG_FINISH_TIMEOUT_SECONDS
                                                        repeats:NO
                                                          block:^(NSTimer *t UNUSED) {
                                                            drag_finish_timer = nil;
                                                            fire_drag_finished();
                                                          }];
}
    ...
    if (thumbnail && thumbnail->pixels && window) {
        ...
        if (!_glfw.ns.drag_session) {
            ...
            if (glfw_window && glfw_window->ns.view) {
                ...
                NSPoint cursor = NSMakePoint(glfw_window->virtualCursorPosX, contentRect.size.height - glfw_window->virtualCursorPosY);
...
add_uri_list_drag_items(
    _GLFWwindow *window, NSMutableArray<NSDraggingItem *> *dragItems, const char *uri_list, size_t uri_list_sz, bool use_promises, const GLFWimage *thumbnail) {
...
add_drag_items(_GLFWwindow *window, NSMutableArray<NSDraggingItem *> *dragItems, GLFWDragSourceItem *mime_item, const GLFWimage *thumbnail) {
    ...
    if (mime_item->optional_data) {
        ...
        GLFWFilePromiseProviderDelegate *delegate = [[[GLFWFilePromiseProviderDelegate alloc] initWithWindow:window
                                                                                                    mimeType:mime_item->mime_type
                                                                                                    fileName:NULL
                                                                                                       isDir:NO
                                                                                                  instanceId:_glfw.drag.instance_id] autorelease];
        NSFilePromiseProvider *provider = [[[NSFilePromiseProvider alloc] initWithFileType:utiString delegate:delegate] autorelease];
        w = provider;
    }
...
_glfwPlatformStartDrag(_GLFWwindow *window, const GLFWimage *thumbnail) {
    @autoreleasepool {
        ...
        if (!event || ([event type] != NSEventTypeLeftMouseDown && [event type] != NSEventTypeLeftMouseDragged)) {
            ...
            NSPoint windowPos = NSMakePoint(window->virtualCursorPosX, contentRect.size.height - window->virtualCursorPosY);
...
- (bool)is_mimetype:(const char *)q {
    return strcmp(q, mimeType) == 0;
}
...
- (void)promised_data_ready:(const char *)path sz:(size_t)sz type:(int)type {
    ...
    switch (type) {
        ...
        default: {
            ...
            NSDirectoryEnumerator<NSURL *> *enumerator = [fileManager enumeratorAtURL:srcURL
...
- (void)request_drag_data {
    if (instanceId != _glfw.drag.instance_id) {
...
- (instancetype)initWithWindow:(_GLFWwindow *)initWindow
                      mimeType:(const char *)mime
                      fileName:(const char *)name
                         isDir:(bool)is_dir
                    instanceId:(GLFWid)instance_id {
    self = [super init];
    if (self) {
...
- (void)filePromiseProvider:(NSFilePromiseProvider *)filePromiseProvider
    ...
    NSError *error = nil;
    bool ok = is_directory ? [[NSFileManager defaultManager] createDirectoryAtURL:url withIntermediateDirectories:YES attributes:nil error:&error]
...
_glfwPlatformChangeDragImage(const GLFWimage *thumbnail) {
    @autoreleasepool {
        ...
        _GLFWwindow *window = _glfwWindowForId(_glfw.drag.window_id);
        [((NSDraggingSession *)_glfw.ns.drag_session) enumerateDraggingItemsWithOptions:0
