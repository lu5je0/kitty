...
_report_unknown_escape_code(PyObject *dump_callback, id_type window_id, const char *name, const uint8_t *payload) {
    ...
    if (dump_callback) {
        Py_XDECREF(PyObject_CallFunction(dump_callback, "Kss", window_id, "error", buf));
        PyErr_Clear();
    } else
        log_error(
            ERROR_PREFIX " "
...
_report_params(PyObject *dump_callback, id_type window_id, const char *name, int *params, unsigned int count, bool is_group, Region *r) {
    ...
    buf[count ? p - 1 : p] = 0;
    Py_XDECREF(PyObject_CallFunction(dump_callback, "Kss", window_id, name, buf));
    PyErr_Clear();
}
...
#define CALL_ED2(name, a, b)    \
    ...
    if (is_first_char) {
        switch (ch) {
            ...
            IS_ESCAPED_CHAR:
                return false;
            case ESC_RIS: CALL_ED(screen_reset); break;
            case ESC_IND: CALL_ED(screen_index); break;
            case ESC_NEL: CALL_ED(screen_nel); break;
            case ESC_RI: CALL_ED(screen_reverse_index); break;
            case ESC_HTS: CALL_ED(screen_set_tab_stop); break;
            case ESC_DECSC: CALL_ED(screen_save_cursor); break;
            case ESC_DECRC: CALL_ED(screen_restore_cursor); break;
            case ESC_DECKPNM: CALL_ED(screen_normal_keypad_mode); break;
            case ESC_DECKPAM: CALL_ED(screen_alternate_keypad_mode); break;
            default:
        ...
        switch (prev_ch) {
            case '%':
                switch (ch) {
                    case '@': REPORT_ERROR("Ignoring attempt to switch to non-utf8 encoding"); break;
                    case 'G': REPORT_ERROR("Ignoring attempt to switch to utf8 encoding as we are always utf-8"); break;
                    default: REPORT_ERROR("Unhandled Esc %% code: 0x%x", ch); break;
                }
                break;
            case '#':
                if (ch == '8') {
                    CALL_ED(screen_align);
                } else {
            ...
            case ')':
                switch (ch) {
                    ...
                    case '0':
                    case 'U':
                    case 'V': CALL_ED2(screen_designate_charset, prev_ch - '(', ch); break;
                    default: REPORT_ERROR("Unknown charset: 0x%x", ch); break;
                }
                break;
            case ' ':
                switch (ch) {
                    case 'F':
                    case 'G': REPORT_ERROR("Ignoring attempt to turn on/off C1 controls as we only support C0 controls"); break;
                    default: REPORT_ERROR("Unhandled ESC SP escape code: 0x%x", ch); break;
                }
                break;
            default: REPORT_ERROR("Unhandled charset related escape code: 0x%x 0x%x", prev_ch, ch); break;
        }
        return true;
    }
#undef CALL_ED
...
continue_osc_52(PS *self) {
    ...
    self->buf[self->read.pos++] = '5';
    self->buf[self->read.pos++] = '2';
...
#define END_DISPATCH \
    ...
    switch (code) {
        ...
        case 2:
            ...
            END_DISPATCH
        case 5:
        case 105: REPORT_ERROR("Ignoring OSC 5/105, used by XTerm to change special colors used for rendering bold/italic/underline"); break;
        case 6:
        case 106: { // report only once as this is used by benchmark kitten causing log spam
        ...
        case 7:
            ...
            break;
        case 8: dispatch_hyperlink(self, (char *)buf + i); break;
        case 9:
        case 99:
        case 777:
        case 1337:
        ...
        case 16:
        case 18:
            REPORT_ERROR("Ignoring OSC 13,14,15,16 and 18 used for pointer and Textronic colors by XTerm");
            break;
            break;
        case 10:
        case 11:
        ...
        case 60:
        case 61: REPORT_ERROR("Ignoring OSC 60/61 used for query control in XTerm"); break;
        case TEXT_SIZE_CODE: parse_multicell_code(self, buf + i, limit - i); break;
        case DND_CODE: parse_dnd_code(self, buf + i, limit - i); break;
        case 133:
#ifdef DUMP_COMMANDS
            START_DISPATCH
        ...
        default: REPORT_UKNOWN_ESCAPE_CODE("OSC", buf); break;
    }
#undef DISPATCH_OSC
...
#define dispatch(prefix, func, delta)                                                \
    ...
    dispatch("overlay-ready|", handle_overlay_ready, 0) dispatch("kitten-result|", handle_kitten_result, 0) dispatch("print|", handle_remote_print, 0)
        dispatch("echo|", handle_remote_echo, 0) dispatch("ssh|", handle_remote_ssh, 0) dispatch("ask|", handle_remote_askpass, 0)
            dispatch("clone|", handle_remote_clone, 0) dispatch("edit|", handle_remote_edit, 0)
...
dispatch_dcs(PS *self, uint8_t *buf, size_t bufsz, bool is_extended UNUSED) {
    ...
    switch (buf[0]) {
        ...
        case '@':
            ...
            break;
        default: REPORT_UKNOWN_ESCAPE_CODE("DCS", buf); break;
    }
}
    ...
    while (*pos < sz) {
        ...
        *pos += 1;
        switch (csi->state) {
            case CSI_START:
                switch (ch) {
                    case CSI_NORMAL_MODE_EMBEDDINGS: dispatch_single_byte_control(self, ch); break;
                    case ';':
            ...
            case CSI_POST_SECONDARY:
                switch (ch) {
                    ...
                    case CSI_TRAILER:
                        ...
                        csi->trailer = ch;
                        break;
                    default: REPORT_ERROR("Invalid character in CSI: %s (0x%x), ignoring the sequence", csi_letter(ch), ch); break;
                }
                return true;
            case CSI_BODY:
                switch (ch) {
...
parse_region(const ParsedCSI *csi, Region *r) {
    switch (csi->num_params) {
        case 0: return 0;
        case 1: r->top = csi->params[0]; return 1;
        case 2:
            r->top = csi->params[0];
            r->left = csi->params[1];
            return 2;
        case 3:
            r->top = csi->params[0];
            r->left = csi->params[1];
            r->bottom = csi->params[2];
            return 3;
        default:
            r->top = csi->params[0];
            r->left = csi->params[1];
            r->bottom = csi->params[2];
            r->right = csi->params[3];
            return 4;
    }
}
            ...
            case NORMAL:
                ...
                switch (csi->params[pos]) {
                    ...
                    case DECORATION_FG_CODE:
                        ...
                        num_params = 1;
                        break;
                    default: num_params++; break;
                }
                break;
            case SUB_PARAMS:
                switch (csi->is_sub_param[pos]) {
                    case true: num_params++; break;
                    case false:
                        SEND_SGR;
                        pos--;
                        break;
            ...
            case COLOR:
                switch (csi->params[pos]) {
                    ...
                    case 5:
                        ...
                        num_params++;
                        break;
                    default: REPORT_ERROR("SGR escape code has unknown color type: %d ignoring the full code", csi->params[pos]); return false;
                }
                break;
            case COLOR1:
                num_params++;
...
static void
screen_cursor_up2(Screen *s, unsigned int count) {
    screen_cursor_up(s, count, false, -1);
}
static void
screen_cursor_back1(Screen *s, unsigned int count) {
    screen_cursor_move(s, count, -1, false);
}
static void
...
#define NO_MODIFIERS(modifier, special, special_msg)                                      \
    ...
    switch (self->csi.trailer) {
        case ICH: NO_MODIFIERS(self->csi.secondary, ' ', "Shift left escape code not implemented"); CALL_CSI_HANDLER1(screen_insert_characters, 1);
        case REP: CALL_CSI_HANDLER1(screen_repeat_character, 1);
        case CUU: NO_MODIFIERS(end_modifier, ' ', "Shift right escape code not implemented"); CALL_CSI_HANDLER1(screen_cursor_up2, 1);
        case CUD:
        case VPR: CALL_CSI_HANDLER1(screen_cursor_down, 1);
        case CUF:
        case HPR: CALL_CSI_HANDLER1(screen_cursor_forward, 1);
        case CUB: CALL_CSI_HANDLER1(screen_cursor_back1, 1);
        case CNL: CALL_CSI_HANDLER1(screen_cursor_down1, 1);
        case CPL: CALL_CSI_HANDLER1(screen_cursor_up1, 1);
        case CHA:
        case HPA: CALL_CSI_HANDLER1(screen_cursor_to_column, 1);
        case VPA: CALL_CSI_HANDLER1(screen_cursor_to_line, 1);
        case CBT: CALL_CSI_HANDLER1(screen_backtab, 1);
        case CHT: CALL_CSI_HANDLER1(screen_tabn, 1);
        case CUP:
        case HVP: CALL_CSI_HANDLER2(screen_cursor_position, 1, 1);
        case ED: CALL_CSI_HANDLER1P(screen_erase_in_display, 0, '?');
        case EL: CALL_CSI_HANDLER1P(screen_erase_in_line, 0, '?');
        case IL: CALL_CSI_HANDLER1(screen_insert_lines, 1);
        case DL: CALL_CSI_HANDLER1(screen_delete_lines, 1);
        case DCH:
        ...
        case 'W':
            ...
            break;
        case ECH: CALL_CSI_HANDLER1(screen_erase_characters, 1);
        case DA: CALL_CSI_HANDLER1S(report_device_attributes, 0);
        case TBC: CALL_CSI_HANDLER1(screen_clear_tab_stop, 0);
        case SM: handle_mode(self); break;
        case RM: handle_mode(self); break;
        case DSR: CALL_CSI_HANDLER1P(report_device_status, 0, '?');
        case 's':
        ...
        case 't':
            ...
            if (start_modifier || end_modifier) {
                REPORT_ERROR(
                    "Unknown CSI t sequence with start and end modifiers: '%c' '%c', %u parameters and first parameter: %d",
                    start_modifier,
                    end_modifier,
                    num_params,
                    params[0]);
            ...
            switch (params[0]) {
                ...
                case 23:
                    ...
                    break;
                default: REPORT_ERROR("Unknown CSI t window manipulation sequence with %u parameters and first parameter: %d", num_params, params[0]); break;
            }
            break;
        case 'u':
            if (!start_modifier && !end_modifier && !num_params) {
        ...
        case 'r':
            ...
            break;
        case 'x':
            if (!start_modifier && end_modifier == '*') { CALL_CSI_HANDLER1(screen_decsace, 0); }
        ...
        case DECSCUSR:
            ...
            break;
        case SU: NO_MODIFIERS(end_modifier, ' ', "Select presentation directions escape code not implemented"); CALL_CSI_HANDLER1(screen_scroll, 1);
        case SD:
            if (!start_modifier && end_modifier == '+') {
        ...
        case DECSTR:
            switch (end_modifier) {
                ...
                case '!': // Soft reset
                    ...
                    break;
                default: REPORT_ERROR("Unknown CSI p sequence with start and end modifiers: '%c' '%c'", start_modifier, end_modifier); break;
            }
            break;
        case 'm':
            if (!start_modifier && !end_modifier) {
        ...
        default:
            REPORT_ERROR(
                "Unknown CSI code: '%c' with start_modifier: '%c' and end_modifier: '%c' and parameters: '%s'",
                code,
                start_modifier,
                end_modifier,
                repr_csi_params(params, num_params));
    }
#undef num_params
#undef code
#undef params
...
dispatch_apc(PS *self, uint8_t *buf, size_t bufsz, bool is_extended UNUSED) {
    if (bufsz < 2) return;
    switch (buf[0]) {
        case 'G': parse_graphics_code(self, buf, bufsz); break;
        default: REPORT_ERROR("Unrecognized APC code: 0x%x", buf[0]); break;
    }
}
...
dispatch_pm(PS *self UNUSED, uint8_t *buf, size_t bufsz, bool is_extended UNUSED) {
    if (bufsz < 2) return;
    switch (buf[0]) {
        default: REPORT_ERROR("Unrecognized PM code: 0x%x", buf[0]); break;
    }
}
...
dispatch_sos(PS *self UNUSED, uint8_t *buf, size_t bufsz, bool is_extended UNUSED) {
    if (bufsz < 2) return;
    switch (buf[0]) {
        default: REPORT_ERROR("Unrecognized SOS code: 0x%x", buf[0]); break;
    }
}
...
#define consume(x)                                               \
    ...
    switch (self->vte_state) {
        ...
        case VTE_CSI:
            if (consume_csi(self)) {
                self->read.consumed = self->read.pos;
                if (self->csi.is_valid) dispatch_csi(self);
                SET_STATE(NORMAL);
            }
            break;
...
#ifdef DUMP_COMMANDS
void
parse_worker_dump(void *p, ParseData *pd, bool flush) {
    run_worker(p, pd, flush);
}
#else
void
parse_worker(void *p, ParseData *pd, bool flush) {
    run_worker(p, pd, flush);
}
#endif
...
static PyGetSetDef getsetters[] = {
    {"vte_state", (getter)current_state, NULL, "The VTE parser state", NULL}, {NULL} /* Sentinel */
};
...
PyTypeObject Parser_Type = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "fast_data_types.Parser",
...
alloc_vt_parser(id_type window_id) {
    ...
    if (self != NULL) {
        ...
        if ((intptr_t)state->buf % BUF_EXTRA != 0) {
            Py_CLEAR(self);
            PyErr_SetString(PyExc_TypeError, "PS->buf is not aligned");
            return NULL;
        }
        if ((ret = pthread_mutex_init(&state->lock, NULL)) != 0) {
            Py_CLEAR(self);
            PyErr_Format(PyExc_RuntimeError, "Failed to create Parser lock mutex: %s", strerror(ret));
            return NULL;
        }
