...
screen_draw_overlay_line(Screen *self) {
    ...
    // fork: pre-edit text is rendered as plain text, without the italic and dashed underline
    // upstream uses to mark it. Override upstream's choice right after it instead of editing the
    // two lines above, so merges of upstream master keep auto-resolving. Pre-edit text is then
    // distinguished only by preedit_foreground/preedit_background; the save/restore below still
    // round-trips the original SGR.
    self->cursor->sgr.italic = false;
    self->cursor->sgr.decoration = 0; // no underline
    ...
    // inherited from whatever the application last drew at the cursor. The absence of decoration is
    // deterministic so it is kept on top of the fork colors.
...
static PyGetSetDef getsetters[] = {
    GETSET(in_bracketed_paste_mode) GETSET(color_preference_notification) GETSET(auto_repeat_enabled) GETSET(focus_tracking_enabled)
        GETSET(in_band_resize_notification) GETSET(paste_events) GETSET(cursor_visible) GETSET(cursor_key_mode) GETSET(disable_ligatures)
            GETSET(render_unfocused_cursor){"ime_disabled", (getter)ime_disabled_get, (setter)ime_disabled_set, NULL, NULL},  // fork-local, see fork-ime.h
    {NULL} /* Sentinel */
};
