...
| `kitty/boss.py` | `titlebar_tab_activate/close/new/drop/detach/drag_out` handler；drop/detach 开头 `set_tab_being_dragged()` 取消挂起的 DND handoff（松键快于异步缩略图回调时，implicit grab 已失效，晚到的 start_drag 会 EPERM）；**撕下窗口错位摆放**（修“拖出的新窗口完全盖住原窗口”）：新增 `_position_for_detached_window()`（源窗口位置 +40,+40，并钳制在所在显示器 work area 内；Wayland 客户端无法设窗口位置直接返回 None 交给 compositor），`_move_tab_to()` 加可选 `x`/`y` 参数（新建 OS window 时未显式传入就用上面的计算值），`_move_window_to()` 的 `'new'` 分支同样套用。合并冲突时保留 |
| `kitty/tabs.py` | `native_titlebar_tabs_supported()`（`is_macos or is_wayland()`）、`use_native_titlebar_tabs` 属性、`update_native_titlebar_tabs()`（带 `last_native_titlebar_tabs_data` 去重缓存：一次切 tab 会触发 2–3 次 `mark_tab_bar_dirty`，相同数据不重推原生层；`apply_options` 里清缓存强制重推，因为 bar 颜色/appearance 是 C 侧推送时才读 options）、`tab_bar_hidden` 计算、`apply_options` 配置重载支持；**改了上游 `on_tab_drop_move` 3 行**：`window_geometry` 访问前加 `laid_out_once` 判断（native tabs 下网格 tab bar 从不 layout，否则跨窗口拖拽 AttributeError），合并冲突时保留该 guard；`start_tab_drag` 里追加几行：把截图缩略图的 alpha 强制为 0xff（GL framebuffer 的 alpha 对终端内容无意义、大面积为 0，Wayland DND 缩略图按它 premultiply 会把内容变透明），合并时保留 |
...
| `glfw/wl_titlebar_tabs.{c,h}` | **Wayland 核心实现，纯 fork 专属文件，零冲突**：按 `window->id` 的模块内状态链表、macOS 同款布局算法、4×4 子采样抗锯齿绘制、命中测试与点击语义、`glfwWaylandSetTitlebarTabs` 导出（按 tab_id diff 保动画状态；强制 CSD、`visible_titlebar_height` 提到 33（frac(33×scale)<0.5 时 34，见下）、立即重设 opaque region）。动画引擎：5 种 0.18s ease-out（位置/宽度、淡入/淡出、hover、变 active 配色）、文字 alpha 蒙版缓存、16ms 单例 timer + 动画期间 `wl_subsurface_set_desync`；非自定义色时用实测 macOS 标题栏色（深色聚焦 #393A39），`forced_appearance` 尊重 `macos_titlebar_color light/dark` |
...
| `glfw/wl_window.c` | `update_regions()` 里 4 行：tabs 激活时 opaque region 挖掉底部两个 10×10 逻辑 px 角（配合 GL 圆角）；include 1 行；DND 三处钩子（纯新增行）：`_glfwWaylandConfirmDragSession()` 里 1 行 `wl_titlebar_tabs_dnd_started()`（compositor 接管指针后清栏内拖拽状态/ghost，否则 ghost 冻在内容区、被拖 tab 一直隐藏）；`_glfwPlatformChangeDragImage()` 里 pending buffer 交换（首次 xdg configure 的 `map_drag_toplevel()` 晚于 ChangeDragImage 时会用旧 idx0 缩略图覆盖新图，表现为标题条被拉伸的灰块）；drag toplevel 路径 4 行 `debug_input` 诊断（仅 `--debug-input` 时打印）；**DND 表面解析**：新增 `fork_window_for_drop_surface()`（`drag_enter` 上方），`drag_enter`/`drag_leave`/`motion`/`drop`/`drop_data_available`/`_glfwPlatformReadAvailableDropData` 六处的表面→窗口匹配改用它（见下） |
...
| `kitty/shaders/slang.py` | 内存 shader 先写入临时 `.slang` 文件再编译，兼容只接受普通输入文件的 Nix `shader-slang`；顶点与片段仍并行编译；将其输出的 C 风格数组初始化转换为 GLSL 1.40 数组构造语法 |
| `kitty_tests/slang.py` | 覆盖 Nix `shader-slang` 数组初始化兼容转换 |
...
- 标题栏高度：正常 CSD 是 24 逻辑 px；有 tab 时首次 `glfwWaylandSetTitlebarTabs` 会把**该窗口**的 `decs.metrics` 提到 33（macOS 实测 28：1px 浅描边叠在 bar 顶 + 1px bar + 24px tab + 2px bar，×7/6 取整 33）。并把 `decs.for_window_state.width` 清零强制重建 shm 缓冲。没改 `csd_initialize_metrics()` 本身。**分数缩放下 33 会再 +1**（如 1.7）：kwin 把标题栏 subsurface 和主 surface 按各自逻辑高度独立取整成物理 px，frac(33×scale)<0.5 时两者之和比 maximize 区少 1 物理 px，窗口底边露一条壁纸缝（溢出 1px 则藏进面板后面不可见，所以把取整方向扳成向上：frac∈(0,0.5) 时 vth=34）；
- **窗口外框 = 1 物理 px 近黑轮廓 + 内缩 1px 的浅色描边**（macos.png @2x 实测，外→内物理行：最外圈近黑(2~16)；顶边 {129, 113} 即 {0.36, 0.28}、侧/底列平坦 {81, 81} 即 0.20；**四角弧和直边同宽**（不能加宽，否则视觉上圆角发“厚”），alpha 沿弧从顶边值旋转到侧边值，`topness = dy/√(dx²+dy²)`）：标题栏部分（顶边+顶角弧+侧列）由 `wl_titlebar_tabs.c` 的 `draw_titlebar_border()` 画进 CSD buffer（在 `round_top_corners` 之前，暗轮廓 `ow=1`、浅描边从 `ow` 开始）；内容区左右/底边+底角弧由 `kitty/shaders.c` 的 `draw_bottom_corner_masks()` 里的描边 pass 画（`kitty/shaders/corner_mask.slang` 的 `border_color` uniform：a==0 走原裁切模式，a>0 走描边/实心模式，`GL_ONE/GL_ONE_MINUS_SRC_ALPHA`；暗轮廓用 premultiplied 黑 a=1）。
- **圆角的边缘羽化 `WINDOW_CORNER_FEATHER = 1.5` 逻辑 px**（修“弧上暗线一路纯黑、mac 是渐变成灰”）：把两边截图还原成物理像素网格后实测，mac 直边暗线 17~37、**弧上最暗只有 72~90**（比标题栏底色 58 还亮，说明那些像素是半透明的、透出了桌面）；我们原本是硬 1px 面积覆盖 AA，弧上暗线 15~50。根因是 mac 把窗口边缘在圆角处淡出约 1.5 逻辑 px，暗线外半被这层渐隐吃掉才发灰。实现：`round_top_corners()` 改成以半径为中心的线性渐隐 `clamp(0.5 + (r - dist)/feather)`（不再用 `circle_coverage`）；`corner_mask.slang` 的裁切模式用同一公式，**feather 走 `rect.w`**（裁切模式下该分量原本未用）。**并且 `draw_bottom_corner_masks()` 的 pass 顺序必须是“先描边、后裁角”**（原本相反）：描边用 `GL_ONE/GL_ONE_MINUS_SRC_ALPHA` 会把已裁掉的 alpha 重新加满，先裁后描羽化就白做了。合并时保留这个顺序。
...
- **DND 交接生命周期**（修“拖 tab 出现冻住的 ghost + 拉伸灰块缩略图”）：栏内拖拽越出 bar ±40 逻辑 px 后发 `GLFW_TITLEBAR_TAB_DRAG_OUT`，Python 异步截图后 `start_drag_with_data` 起 mime DND（kwin 上走 `xdg_toplevel_drag_v1`，拖拽缩略图是个真 toplevel）。compositor 接受 start_drag 后会接管指针，客户端再也收不到 motion/release——所以 `_glfwWaylandConfirmDragSession()`（wl_window.c）里调 `wl_titlebar_tabs_dnd_started()` 清掉栏内 dragging/drag_out/ghost 并重画 bar；在此之前（handoff 异步窗口期）栏内 ghost 继续跟手，与 macOS 手感一致。缩略图两级（idx0 标题条 / idx1 标题+截图）切换走 `_glfwPlatformChangeDragImage`，它和首次 configure 的 `map_drag_toplevel()` 有竞态（map 晚到会 attach 旧 buffer），现在 ChangeDragImage 在 pending 时直接交换 `toplevel_buffer`。另：截图字节 alpha 大面积为 0，tabs.py `start_tab_drag` 里强制 0xff 否则内容透明。
- **tab 拖到别的窗口的 tab 栏上不生效**（根因：原生 tab 栏画在 CSD titlebar **子表面**里，跨窗口拖拽的 `wl_data_device` enter/motion/drop 由 compositor 发给指针下的表面——即 titlebar 子表面；上游 `drag_enter`/`motion`/`drop`/`drag_leave` 只用 `window->wl.surface == surface` 匹配主表面，子表面永远匹配不上，事件被静默丢弃，所以只有落在主表面（非 tab 栏区）的拖放生效）。修法：`wl_window.c` 的 `fork_window_for_drop_surface()`（纯新增）先按主表面匹配，再遍历每个窗口的 9 个 CSD 子表面（titlebar + 8 个 shadow），命中时回传子表面的 `x`/`y` 位置偏移；六处表面→窗口匹配（`drag_enter`/`drag_leave`/`motion`/`drop`/`drop_data_available`/`_glfwPlatformReadAvailableDropData`）改用它，带坐标的两处把 surface 局部坐标加上偏移转成主表面坐标（titlebar 的 y 偏移为负，坐标落在主表面坐标系里可以为负，`on_mouse_position_update`/`in_tab_bar` 判断都能安全处理）。后两处是异步取数路径，不改的话 enter/motion 通了但 drop 数据读不到（`-ENOENT`）/被 `destroy_data_offer` 丢弃。合并冲突时六处匹配的替换和 helper 都要保留；DROP 事件本身用 `GLFW_DROP_DROP, 0, 0` 不带坐标，kitty 侧用的是最后一次 MOVE 的坐标（`last_drag_event`），所以只要 motion 坐标转对了即可。
...
- **`enable` 会清掉 compositor 侧的 cursor rectangle**（现象：刚开 kitty 或刚进 vim，**第一次**组词候选框出现在窗口左上角 (0,0)，commit 一次之后就正常了）。`zwp_text_input_v3_enable` 按协议会 reset 全部状态（含 `set_cursor_rectangle`），而上游只在 `text_input_enter` 里补发缓存矩形，`GLFW_IME_UPDATE_FOCUS` 的 enable 分支不补发；更要命的是 `GLFW_IME_UPDATE_CURSOR_POSITION` 按 `last_cursor_*` 去重，enable 之后矩形没变就不再推送，而组词期间上报的矩形又是刻意和组词前一致的（见上面 preedit 锚定），于是整个第一次组词 compositor 手里都没有矩形。触发 enable 的路径都会中招：`Window.focus_changed()` → `update_ime_position_for_window(id, False, 1)`（切 pane / 窗口获得焦点）、nvim 进插入模式解除 inhibit（`fork_ime_set_disabled` 走同一入口）、以及 compositor 的 text-input `enter` 和 `wl_keyboard.enter` 到达顺序反过来的时候——所以是"有时候"。修法（`wl_text_input.c`，纯新增）：fork-local `fork_cursor_rect_synced` 标志记录"compositor 手里还有我们的矩形"，用**同名宏包住** `zwp_text_input_v3_enable/disable/set_cursor_rectangle` 三个生成函数来维护它（宏体内的同名调用不会被二次展开，因此上游所有调用点一行不动，上游以后新增 enable 调用点也自动覆盖）；FOCUS 的 enable 分支后补 1 行 `fork_ime_resend_cursor_rectangle()`（镜像 `text_input_enter`，让矩形立即恢复而不必等下一帧渲染）；`CURSOR_POSITION` 开头 1 行 `if (!fork_cursor_rect_synced) last_cursor_width = 0;` 破掉去重强制重推。合并冲突时这三个宏和两处调用都要保留。
...
| `glfw/wl_text_input.c` | Wayland 生效路径（**全部纯新增，零删改**）：static `fork_ime_inhibited`、`text_input_enter` 早退 1 行、FOCUS case 顶部拦截 1 行、新 helper `fork_ime_force_disable()`、文件末尾导出 `glfwWaylandSetIMEInhibited()`；另加 cursor rectangle 保同步：`fork_cursor_rect_synced` + 包住三个 `zwp_text_input_v3_*` 请求的同名宏 + `fork_ime_resend_cursor_rectangle()` + FOCUS/CURSOR_POSITION 各 1 行调用（修第一次组词候选框跑到左上角）。冲突时全部保留 |
